...
def prepare_app_context(db: Db) -> None:
    ...
    if not ctx_present:
        ...
        db.query_no_return(
            f"SELECT hive.app_create_context('{SCHEMA_NAME}', '{SCHEMA_NAME}', _is_forking => TRUE, _stages => {synchronization_stages} );"
        )  # is-forking=TRUE, process reversible blocks; HFM auto-rewinds registered tables on microfork
        log.info("Application context creation done.")
    else:
        log.info("Found existing context, set to forking.")
        db.query_no_return(
            f"SELECT hive.app_context_set_forking('{SCHEMA_NAME}');"
        )  # if existing context, make it forking
